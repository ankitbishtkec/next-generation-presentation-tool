var Carousel=function(rad,_images,w,h)
{
	...
	for (var i=0;i<this._images.length;i++)
	{
		...
		this.imgs[i].src=this._images[i].url;
	}
	this.anglePer=2*Math.PI/this._images.length;
}
...
{
	scope.howMany++;
	if (scope.howMany==scope._images.length)
	{
		for (var i=0; i<scope._images.length;i++)
		{
			// text caption
			var size=(0.4)*(scope.w/scope._images[i].url.length);
			var height=2;
			var text3d = new THREE.TextGeometry( scope._images[i].url,{size: size, height: height, curveSegments: 2, font:'helvetiker'});
			...
			// image plane
			var texture=new THREE.Texture(scope.imgs[i]); //THREE.ImageUtils.loadTexture(scope._images[i].url);
			texture = new THREE.ImageUtils.loadTexture(scope._images[i].url);
			var plane = new THREE.Mesh( new THREE.PlaneGeometry( scope.w, scope.h, 3, 3 ), new THREE.MeshBasicMaterial( { map: texture, overdraw: true, side: THREE.DoubleSide  } ) );
