/**
 * Public License: Please see licenses\fabricjsLicense.txt for details.
 */
...
//SISO added
fabric.isTouchSupported = ('ontouchstart' in window);
...
(function (global) {
  ...
  extend(fabric.StaticCanvas.prototype, /** @scope fabric.StaticCanvas.prototype */ {
    ...
    add: function () {
      ...
      this.renderOnAddition && customRenderAll();
      return this;
    },
	...
	/**
     * checks if object/ figure lies in viewable area of dom canvas element // SISO
     * @method if 'forceDraw' flag is set it always returns true 
     * @return true if it is inside of the viewable area else false, if 'forceDraw' flag is set it always returns true 
     * @chainable
     */
    inViewableCanvasArea: function ( objectToCheck, forceDraw ) {
      if( forceDraw )
		return true;
	  //console.log(objectToCheck._hypotenuse +"\n" + this.getWidth() +"\n" + this.getHeight());
	  var limitLeft = 0 - objectToCheck._hypotenuse;
	  var limitRight = this.getWidth() + objectToCheck._hypotenuse;
	  var limitTop = 0 - objectToCheck._hypotenuse;
	  var limitBottom = this.getHeight() + objectToCheck._hypotenuse;
	  if( ( objectToCheck.getLeft() > limitLeft ) && ( objectToCheck.getLeft() < limitRight ) )
		if( ( objectToCheck.getTop() > limitTop ) && ( objectToCheck.getTop() < limitBottom ) )
			return true;
		else
			return false;
	  else
      ...
      if (length) {
        for (var i = 0; i < length; ++i) {
          if (!activeGroup ||
			...
			if(this.inViewableCanvasArea( this._objects[i] , false))
				
				{
					if( this._objects[i] && this._objects[i].name )
						if( this._objects[i].name == "superGrid" )
							superGrid = this._objects[i];
					if(superGrid)
						if( this._objects[i] == superGrid )
							superGrid.setOpacity(0);
						
					this._draw(canvasToDrawOn, this._objects[i]);
					
					if( this._objects[i] && this._objects[i].name )
						if( this._objects[i].name == "mainInfiniteRect" )
								if( superGrid && gridEnable )
								{
									//manageSuperGrid();
									this.drawLattice( superGrid.getLeft(), superGrid.getTop(), (150 * canvasScaleFactor) );
									//this.drawLattice( 0, 0, 150 );
								}
				}
          }
        }
...
//draws rectangular grid	
 drawLattice: function ( centerx, centery, gridOffset) {
		
	var canvasWidth = mainFabricCanvas.getWidth();
    var canvasHeight = mainFabricCanvas.getHeight();
	...
	//context.fillStyle= "rgba(0, 0, 0, 0.0)";//black transparent 
	//context.strokeStyle = "rgba(0, 0, 0, 0.0)";//black transparent
    //context.stroke();
	
	context.lineWidth = 1;
	context.fillStyle= "rgba(0, 0, 0, 1.0)";//black   
	context.strokeStyle = "rgb(0, 0, 0, 1.0)";//black
	
	//towards right x vertical line
	var startX = centerx;
	var startY = centery;
	...
	//towards bottom y horizontal line
	while(1)
	{
		if(startY > canvasHeight)
			break;
        context.lineTo( canvasWidth, startY );
		startY = startY + gridOffset;	
		context.moveTo( 0 , startY);	
	}
...
(function() {
  ...
  var InteractiveMethods = /** @scope fabric.Canvas.prototype */ {
    ...
    _initEvents: function () {
	  ...
	  // SISO : Added custom mouse wheel event in fabric
	  this._onMouseWheelScroll= function (e) {
		_this.__onMouseWheelScroll(e);
      ...
      if (fabric.isTouchSupported) {
		...
		*/
		// SISO : capture mouse wheel
		if((/Firefox/i.test(navigator.userAgent)))
			addListener(this.upperCanvasEl, 'DOMMouseScroll', this._onMouseWheelScroll);//firefox
		else
			addListener(this.upperCanvasEl, 'mousewheel', this._onMouseWheelScroll); // chrome
      }
      else {
		/* SISO
	...
	// SISO : added for mouse wheel
	__onMouseWheelScroll: function (e) {
      ...
      this.fire('mouse:scroll', { target: target, e: e });
	},
    ...
    _initWrapperElement: function () {
      ...
      //SISO
	  var resizeWrapperEl = fabric.util.wrapElement(this.wrapperEl, 'div',null);
      fabric.util.setStyle(resizeWrapperEl, {
        width: this.getWidth() + 'px',
        height: this.getHeight() + 'px',
		backgroundImage: "url('./resources/tile.jpg')",
		backgroundRepeat : '',
		left: '0px',
		top: '0px'
      });
      fabric.util.makeElementUnselectable(resizeWrapperEl);
    },
	...
	{
		if(objectsToTexturedCube.length < 6)
		{
			StartTexturedCube();
			if (objectsToTexturedCube.length == 6)
			{
				var l = objectsToTexturedCube.length - 1;
				var referencedObj = objectsToTexturedCube[l];
				referencedObj.isReferenceObj = true;
				objectsToTexturedCube[l].picturedCube = texturedCubeCount;
				var rLeft = referencedObj.getLeft();
				var rTop = referencedObj.getTop();

				
				for (var c = objectsToTexturedCube.length - 1; c >= 0 ; c--)
				{
					objectsToTexturedCube[c].width = 75;
					objectsToTexturedCube[c].height = 75;
					objectsToTexturedCube[c].scale(1);
				}
				...
				var PADDING = 4;
				l--;
				objectsToTexturedCube[l].setLeft((rLeft - w) - PADDING);
				objectsToTexturedCube[l].setTop(rTop);
				objectsToTexturedCube[l].picturedCube = texturedCubeCount;
				l--;
				objectsToTexturedCube[l].setLeft((rLeft + w) + PADDING);
				objectsToTexturedCube[l].setTop(rTop);
				objectsToTexturedCube[l].picturedCube = texturedCubeCount;
				l--;
				objectsToTexturedCube[l].setLeft(rLeft);
				objectsToTexturedCube[l].setTop((rTop - w) - PADDING);
				objectsToTexturedCube[l].picturedCube = texturedCubeCount;
				l--;
				objectsToTexturedCube[l].setLeft(rLeft);
				objectsToTexturedCube[l].setTop((rTop + w) + PADDING);
				...
				l--;
				objectsToTexturedCube[l].setLeft(rLeft);
				objectsToTexturedCube[l].setTop(rTop + 2*(w + PADDING)).setCoords();
				objectsToTexturedCube[l].picturedCube = texturedCubeCount;
				customRenderAll();
				texturedCubeObjArray[texturedCubeCount] = new Array();
				for (var c = 0; c < 6; c++)
				{
					texturedCubeObjArray[texturedCubeCount][c] = objectsToTexturedCube[c];
				}
				texturedCubeCount++;
				objectsToTexturedCube = [];
			}
		}	
	}
	  ...
	  {
		objection=null;
		if(object.isLeftCtrl ||object.isRightCtrl )
		{
			//console.log("isLeftCtrl");
			prevOrNext(object);
			
		}
		else if(object.isShuffle)
		{
		//console.log("isLeftCtr2");
			//return;
		}
			
		else 
		{
		//Fconsole.log("isLeftCtr3");
  ...
  fabric.Object = fabric.util.createClass(/** @scope fabric.Object.prototype */ {
    ...
    toObject: function() {
      ...
      var object = {
		...
		isPathSet:		  this.isPathSet,
		isLeftCtrl:		  this.isLeftCtrl,
		isRightCtrl:      this.isRightCtrl,
    ...
    // Temp fix
	if(property == 'left' )
		if(this.name == 'mainInfiniteRect')
		{
			mainInfiniteRect = this;
			//console.log("all" + this.left + "hhh" + this.top);
		}
		if(this.name == 'superGrid')
		{
			superGrid = this;
			//console.log("all" + this.left + "hhh" + this.top);
		}
...
(function(global) {
  ...
  fabric.Line.fromObject = function(object) {
    ...
    var response = new fabric.Line(points, object);
	response.TagNumber = object.TagNumber;
	response.animating = object.animating;
	response.isPathSet = object.isPathSet;
...
(function(global) {
  ...
  fabric.Circle.fromObject = function(object) {
    var response = new fabric.Circle(object);
	response.id = object.id;
	response.TagNumber = object.TagNumber;
	response.animating = object.animating;
	response.isPathSet = object.isPathSet;
	response.isShuffle = object.isShuffle;
	if(response.isShuffle) {
	response.hasControls = object.hasControls;
	response.lockMovementX = true;
	response.lockMovementY =  true;
	response.lockScalingX = true;
	response.lockScalingY = true;
	response.lockUniScaling = true;
	response.lockRotation = true;
	}
	return response;
  };
...
(function(global) {
  ...
  fabric.Triangle.fromObject = function(object) {
	var response = new fabric.Triangle(object);
	response.TagNumber = object.TagNumber;
	response.animating = object.animating;
	response.isPathSet = object.isPathSet;
	response.isLeftCtrl = object.isLeftCtrl;
	response.isRightCtrl = object.isRightCtrl;
	response.hasControls = object.hasControls;
...
(function(global){
  ...
  fabric.Ellipse.fromObject = function(object) {
    var response =  new fabric.Ellipse(object);
	response.id = object.id;
	response.TagNumber = object.TagNumber;
	response.animating = object.animating;
	response.isPathSet = object.isPathSet;
...
(function(global) {
  ...
  fabric.Rect.fromObject = function(object) {
    var response = new fabric.Rect(object);
	if(object.isFrame)
		response.isFrame = object.isFrame;
	if(object.TagNumber >= 0 )
		response.TagNumber = object.TagNumber;
	response.name = object.name;
	response.id = object.id;
	response.animating = object.animating;
	response.isPathSet = object.isPathSet;
	if(object.hasControls == false)
	{
		response.lockMovementX = true;
		response.lockMovementY = true;
		response.lockScalingX = true;
...
(function(global) {
  ...
  fabric.Path = fabric.util.createClass(fabric.Object, /** @scope fabric.Path.prototype */ {
    ...
    toObject: function() {
      var o = extend(this.callSuper('toObject'), {
		...
		frameName: this.frameName,
		isFrame: this.isFrame,
		isSlideObj: this.isSlideObj,
		slideNum: this.slideNum,
		child1: this.child1,
		child2: this.child2,
		child3: this.child3,
		lockUniScaling: this.lockUniScaling,
		initialChildren: this.initialChildren
      });
      if (this.sourcePath) {
  ...
  fabric.Path.fromObject = function(object) {
   var response = new fabric.Path(object.path, object);
   response.id = object.id;
   response.frameName = object.frameName;
   response.isFrame = object.isFrame;
   response.isSlideObj = object.isSlideObj;
   response.slideNum = object.slideNum;
   response.lockUniScaling = object.lockUniScaling;
   response.initialChildren =object.initialChildren;
   response.child1 = object.child1;
   response.child2 = object.child2;
   response.child3 = object.child3;
   return response;
  };
...
(function(global) {
  ...
  fabric.Image = fabric.util.createClass(fabric.Object, /** @scope fabric.Image.prototype */ {
    ...
    toObject: function() {
      return extend(this.callSuper('toObject'), {
		...
		id: this.id,
		isPathSet: this.isPathSet,
		isPathImg: this.isPathImg,
		isImgOnCanvas: this.isImgOnCanvas,
    ...
    applyFilters: function(callback) {
      ...
      canvasEl.getContext('2d').drawImage(imgEl, 0, 0, imgEl.width, imgEl.height); //SISO added width and height params
     ...
     */
    _render: function(ctx) {
      if(this.themicBackground)//SISO
	  {
			try{
			if(!themePattern || themePattern == "change")
				themePattern = ctx.createPattern(this.getElement(), "repeat");
  ...
  fabric.Image.fromObject = function(object, callback) {
	...
	img.id = object.id;
	img.isPathSet = object.isPathSet;
	img.hasAudio = object.hasAudio;
	img.audioUrl = object.audioUrl;
	//img.hasAudio = object.hasAudio;
	img.animating = object.animating;
	img.isPathSet = object.isPathSet;
	img.isImgOnCanvas = object.isImgOnCanvas;
	img.isPathImg = object.isPathImg;
    /** @ignore */
    img.onload = function() {
 ...
 * @memberOf fabric.Image.filters
 */
fabric.Image.filters.Sepia3 = fabric.util.createClass( /** @scope fabric.Image.filters.Sepia2.prototype */ {
...
(function(global) {
  ...
  fabric.Text = fabric.util.createClass(fabric.Object, /** @scope fabric.Text.prototype */ {
	...
	_setCharAt: function(str,index,chr) {
			...
			return str.substr(0,index) + chr + this._setLines(str.substr(index+1));
	},
	
	_setLines: function(str){	  	  
	  var regexp = / /g;	 
	  var match, matchlast,matches = [] ;
      var newStr = '';
		while ((match = regexp.exec(str)) != null) {					
		  if(match.index >= 100 && matchlast < 100)
		    {
			newStr = this._setCharAt(str,matchlast,'\n');
			}
    ...
    _renderViaNative: function(ctx) {
	  ...
	  for (var i=0;i<lines.length;i++)
	   {
	    var str = this._setLines(lines[i]);
	   textLines.push.apply(textLines,str.split(/\r?\n/));	   
	   }
      ...
      //this.setCoords();// SISO commented it
    },
  ...
  fabric.Text.fromObject = function(object) {
    var response = new fabric.Text(object.text, clone(object));
	response.name = object.name;
	response.id = object.id;
	response.TagNumber = object.TagNumber;
	response.animating = object.animating;
	response.isPathSet = object.isPathSet;
	if(object.name && (object.name.substring(0,4) == "TAG_") ){//&& !object.isPathSet){
		response.lockMovementX = true;
		response.lockMovementY = true;
		response.lockScalingX = true;
		response.lockScalingY = true;
...
(function() {
  ...
  fabric.Image.fromObject = function(object, callback) {
    fabric.util.loadImage(object.src, function(img) {
	  ...
	  oImg.id = object.id;
	  oImg.hasAudio = object.hasAudio;
	  oImg.isPathSet = object.isPathSet;
