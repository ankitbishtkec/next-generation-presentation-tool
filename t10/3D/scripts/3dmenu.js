...
function initilizeContextMenu(baseElement)
{
	menuArguments = {
		Base: baseElement,
		Width: 200,
...
{
	var baseElement = document.getElementById('3DCanvas');
	...
	try{
		...
		var target = baseElement.findTarget(e) ? baseElement.findTarget(e) : baseElement.getActiveObject();
		customContextMenu = new CustomContextMenu(menuArguments);
		customSubContextMenu = new CustomSubContextMenu(menuArguments);
		...
		if(target)
		{
			if (target.name == "mainInfiniteRect" || target.isFrame)
				showDefaultMenu(customContextMenu, baseElement, target);
			else if(!isNotTagArea(target))
				showTagItMenu(customContextMenu, baseElement, target);
			else if(target.name == "video")
				showVideoMenu(customContextMenu, baseElement, target);
			else //For any inserted object
				showFabricObjectMenu(customContextMenu, baseElement, target);
				showSubMenu(customSubContextMenu, baseElement, target);
		}
				/************************************frame/path start******************************/
...
var OnClick = function(Sender, EventArgs)
{
	switch(EventArgs.MenuType)
	{
		...
		/* 3D */
		case 'threeD':
			handleThreeDMenuClick(EventArgs.CommandName, EventArgs.Target);
			break;	
...
function show3DObjectMenu(customMenu)
{
	if(!currentActiveObj.selectedForDepth) {
	customMenu.AddItem('', 'Change Depth', false, 'Change Depth', 'threeD', null);
	customMenu.AddSeparatorItem();}
	if(currentActiveObj.selectedForDepth) {
	customMenu.AddItem('', 'Exit Depth Mode', false, 'Exit Depth Mode', 'threeD', null);
	customMenu.AddSeparatorItem();}
	
	customMenu.AddItem('', 'Rotation Mode', false, 'Rotation Mode', 'threeD', null);
	customMenu.AddSeparatorItem();

	customMenu.AddItem('resources/zoomout.png', 'RotationX', false, 'RotationX', 'threeD', null);
	customMenu.AddSeparatorItem();
	customMenu.AddItem('resources/zoomout.png', 'RotationY', false, 'RotationY', 'threeD', null);
	customMenu.AddSeparatorItem();
	customMenu.AddItem('resources/rotate-clock.png', 'RotationZ', false, 'RotationZ', 'threeD', null);
	customMenu.AddSeparatorItem();
	customMenu.AddItem('resources/rotate-clock.png', 'Stop Rotation', false, 'Stop Rotation', 'threeD', null);
	customMenu.AddSeparatorItem();
	if(!currentActiveObj.selectedForPushpull && !isPictureCube(currentActiveObj)) {
	customMenu.AddItem('resources/zoomin.png', 'Push Pull Editing', false, 'PushPull Mode', 'threeD', null);
	customMenu.AddSeparatorItem();}
	if(currentActiveObj.selectedForPushpull) {
	customMenu.AddItem('resources/zoomin.png', 'Exit Push Pull Editing', false, 'Exit PushPull Mode', 'threeD', null);
	customMenu.AddSeparatorItem();}
	customMenu.AddItem('resources/rotate-clock.png', 'ScaleX', false, 'ScaleX', 'threeD', null);
	customMenu.AddSeparatorItem();
	customMenu.AddItem('resources/rotate-clock.png', 'ScaleY', false, 'ScaleY', 'threeD', null);
	customMenu.AddSeparatorItem();
	customMenu.AddItem('resources/rotate-clock.png', 'ScaleZ', false, 'ScaleZ', 'threeD', null);
	customMenu.AddSeparatorItem();
	customMenu.AddItem('resources/rotate-clock.png', 'Delete', false, 'Delete', 'threeD', null);

}

function handleThreeDMenuClick(command, tagetElement)
{
	var options = {
		rotationDir: null,
		rotationMode: null,
		scaleFDimension: null,
		pushPullMode: null
	};
	...
	switch(command)
	{
		...
		case 'RotationZ':
		case 'Stop Rotation':
			options.rotationDir = command;
			handleContextMenuClickFor3D(options);
		case 'PushPull Mode':
			options.pushPullMode = command;
			handleContextMenuClickFor3D(options);
			break;
		case 'Rotation Mode':
			options.rotationMode = command;
			handleContextMenuClickFor3D(options);
		case 'ScaleX':
		case 'ScaleY':
		case 'ScaleZ':	
			options.scaleFDimension = command; 	
			handleContextMenuClickFor3D(options);
			break;
		case 'Delete':
			delete3DElement(null);
			break;
		case 'Exit PushPull Mode':
			currentActiveObj.selectedForPushpull = false;
			break;
		case 'Change Depth':
			currentActiveObj.selectedForDepth = true;
			break;
		case 'Exit Depth Mode':
			currentActiveObj.selectedForDepth = false;
...
function handleDynamicMenuClick()
{
	...
	switch(id)
	{
		...
		case 'gallery':
			...
			break;
		case 'roomGallery':
			document.getElementById('roomImgGallery').style.display = 'block';
			break;
		case 'dice':
			insertPictureCube();
			break;
		case 'navigation':
			...
			break;
		case 'anaglyph':
			togglePresentationMode("anaglyph");
			break;
